...
class BezierUsage extends Sprite {
	...
	function onPointMoved(?event : Event) : Void {
	}
	...
	function drawBezier(bezierCurve : Bezier, target : Graphics = null, continueDraw : Bool = false) : Void {
		target = target != null ? target : graphics;
		if(!continueDraw)  {
	...
	function drawRectangle(rect : Rect, target : Graphics = null) : Void {
		target = target != null ? target : graphics;
