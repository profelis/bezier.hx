...
class Bezier extends Dynamic, implements IParametric {
	public var start(getStart, setStart) : Point;
	public var control(getControl, setControl) : Point;
	...
	public var area(getArea, never) : Float;
	public var triangleArea(getTriangleArea, never) : Float;
	public var internalCentroid(getInternalCentroid, never) : Point;
	public var externalCentroid(getExternalCentroid, never) : Point;
	public var triangleCentroid(getTriangleCentroid, never) : Point;
	public var bounds(getBounds, never) : Rectangle;
	public var parabolaVertex(getParabolaVertex, never) : Float;
	public var parabolaFocus(getParabolaFocus, never) : Point;
	...
	static var POINT3 : Point = new Point();
	 ...
	 **/
	public function new(start : Point = undefined, control : Point = undefined, end : Point = undefined, isSegment : Bool = true) {
		__isSegment = true;
	 ...
	 * @playerversion Flash 9.0
	 **/
	function initInstance(start : Point = undefined, control : Point = undefined, end : Point = undefined, isSegment : Bool = true) : Void {
		startPoint = (try cast(start, Point) catch(e) null) || new Point();
		controlPoint = (try cast(control, Point) catch(e) null) || new Point();
	...
	public function setControl(value : Point) : Point {
		controlPoint = value;
		return value;
	}
	...
	public function asLine() : Line {
		var startToControlVector : Point = POINT0;
		startToControlVector.x = control.x - start.x;
		startToControlVector.y = control.y - start.y;
		var startToEndVector : Point = POINT1;
		startToEndVector.x = end.x - start.x;
		startToEndVector.y = end.y - start.y;
		if(startToEndVector.length > PRECISION)  {
			var timeOfControl : Float = startToControlVector.length / startToEndVector.length;
			var isLine : Bool = (Math.abs(startToControlVector.x - timeOfControl * startToEndVector.x) < PRECISION) && (Math.abs(startToControlVector.y - timeOfControl * startToEndVector.y) < PRECISION);
			if(isLine)  {
				if(timeOfControl < 0)  {
	...
	public function getSegmentLength(time : Float) : Float {
		...
		startToControlVector.x = control.x - start.x;
		startToControlVector.y = control.y - start.y;
		var diagonalVector : Point = POINT1;
		diagonalVector.x = start.x - 2 * control.x + end.x;
		diagonalVector.y = start.y - 2 * control.y + end.y;
		var startToControlLenght : Float = startToControlVector.length;
		var startToControlLenghtPower2 : Float = startToControlLenght * startToControlLenght;
		var controlToStartMultiplyMainDiagonal : Float = 2 * (startToControlVector.x * diagonalVector.x + startToControlVector.y * diagonalVector.y);
		var diagonalLenght : Float = diagonalVector.length;
		var diagonalLenghtPower2 : Float = diagonalLenght * diagonalLenght;
		...
		if(diagonalLenght == 0)  {
			...
			var integralFrequentPart1 : Float = Math.sqrt(diagonalLenghtPower2 * time * time + controlToStartMultiplyMainDiagonal * time + startToControlLenghtPower2);
			var integralFrequentPart2 : Float = (controlToStartMultiplyMainDiagonal + 2 * diagonalLenghtPower2 * time) / diagonalLenght + 2 * integralFrequentPart1;
			var integralFrequentPart3 : Float = controlToStartMultiplyMainDiagonal / diagonalLenght + 2 * startToControlLenght;
			var integralFrequentPart4 : Float = (startToControlLenghtPower2 - 0.25 * controlToStartMultiplyMainDiagonal * controlToStartMultiplyMainDiagonal / diagonalLenghtPower2);
	...
	public function getTriangleArea() : Float {
		var distanceStartControl : Float = Point.distance(startPoint, controlPoint);
		var distanceEndControl : Float = Point.distance(endPoint, controlPoint);
		var distanceStartEnd : Float = Point.distance(startPoint, endPoint);
		var halfPerimeter : Float = (distanceStartControl + distanceEndControl + distanceStartEnd) / 2;
		var area : Float = Math.sqrt(halfPerimeter * (halfPerimeter - distanceStartControl) * (halfPerimeter - distanceEndControl) * (halfPerimeter - distanceStartEnd));
		return area;
	}
	 ...
	 * 
	 */
	public function getInternalCentroid() : Point {
		var x : Float = (startPoint.x + endPoint.x) * 0.4 + controlPoint.x * 0.2;
		var y : Float = (startPoint.y + endPoint.y) * 0.4 + controlPoint.y * 0.2;
	 ...
	 * @lang rus
	 */
	public function getExternalCentroid() : Point {
		var x : Float = (startPoint.x + endPoint.x) * 0.2 + controlPoint.x * 0.6;
		var y : Float = (startPoint.y + endPoint.y) * 0.2 + controlPoint.y * 0.6;
		return new Point(x, y);
	}
	 ...
	 * @playerversion Flash 9.0
	 */
	public function getTriangleCentroid() : Point {
		var x : Float = (startPoint.x + endPoint.x + controlPoint.x) / 3;
		var y : Float = (startPoint.y + endPoint.y + controlPoint.y) / 3;
		return new Point(x, y);
	}
	...
	public function getBounds() : Rectangle {
		var xMin : Float = Math.min(startPoint.x, endPoint.x);
		var xMax : Float = Math.max(startPoint.x, endPoint.x);
		var yMin : Float = Math.min(startPoint.y, endPoint.y);
		var yMax : Float = Math.max(startPoint.y, endPoint.y);
		var controlToStartVector : Point = POINT0;
		controlToStartVector.x = start.x - control.x;
		controlToStartVector.y = start.y - control.y;
		var diagonalVector : Point = POINT1;
		diagonalVector.x = start.x - 2 * control.x + end.x;
		diagonalVector.y = start.y - 2 * control.y + end.y;
		var extremumTimeX : Float = controlToStartVector.x / diagonalVector.x;
		var extremumTimeY : Float = controlToStartVector.y / diagonalVector.y;
		if((!Math.isNaN(extremumTimeX)) && (extremumTimeX > 0) && (extremumTimeX < 1))  {
			var extremumPointX : Point = getPoint(extremumTimeX);
	 ...
	 * @translator Ilya Segeda http://www.digitaldesign.com.ua
	 **/
	public function getParabolaVertex() : Float {
		var controlToStartVector : Point = POINT0;
		controlToStartVector.x = start.x - control.x;
		controlToStartVector.y = start.y - control.y;
		var diagonalVector : Point = POINT1;
		diagonalVector.x = start.x - 2 * control.x + end.x;
		diagonalVector.y = start.y - 2 * control.y + end.y;
		var diagonalLengh : Float = diagonalVector.length;
		var diagonalLenghtPower2 : Float = diagonalLengh * diagonalLengh;
		var vertexTime : Float = 0.5;
	 ...
	 * 
	 */
	public function getParabolaFocus() : Point {
		var startToControlVector : Point = POINT0;
		startToControlVector.x = control.x - start.x;
		startToControlVector.y = control.y - start.y;
		var diagonalVector : Point = POINT1;
		...
		var vertexTime : Float = -(diagonalVector.x * startToControlVector.x + diagonalVector.y * startToControlVector.y) / diagonalLenghtPower2;
		var parabolaVertex : Point = getPoint(vertexTime);
		var parabolaAxisX : Float = 2 * startToControlVector.x + 2 * vertexTime * diagonalVector.x;
		var parabolaAxisY : Float = 2 * startToControlVector.y + 2 * vertexTime * diagonalVector.y;
		var parabolaAxisLengthPower2 : Float = parabolaAxisX * parabolaAxisX + parabolaAxisY * parabolaAxisY;
		var parabolaAxisLength : Float = Math.sqrt(parabolaAxisLengthPower2);
		var focusLength : Float = parabolaAxisLengthPower2 / diagonalLenght / 4;
		var focusX : Float = parabolaVertex.x - focusLength * parabolaAxisY / parabolaAxisLength;
		var focusY : Float = parabolaVertex.y + focusLength * parabolaAxisX / parabolaAxisLength;
		var parabolaConvexity : Float = (parabolaAxisY * (start.x - parabolaVertex.x) - parabolaAxisX * (start.y - parabolaVertex.y)) * (parabolaAxisY * (focusX - parabolaVertex.x) - parabolaAxisX * (focusY - parabolaVertex.y));
	...
	public function getPoint(time : Float, point : Point = null) : Point {
		...
		var invertedTime : Float = 1 - time;
		var timePower2 : Float = time * time;
		var invertedTimePower2 : Float = invertedTime * invertedTime;
	 ...
	 */
	function getTimesByDistances(distances : Array<Dynamic>) : Array<Dynamic> {
		var startToControlVector : Point = POINT0;
		startToControlVector.x = control.x - start.x;
		startToControlVector.y = control.y - start.y;
		var diagonalVector : Point = POINT1;
		...
		var diagonalLenght : Float = diagonalVector.length;
		var diagonalLenghtPower2 : Float = diagonalLenght * diagonalLenght;
		var integralFrequentPart1 : Float = 4 * startToControlLenghtPower2 - controlToStartMultiplyMainDiagonal * controlToStartMultiplyMainDiagonal / diagonalLenghtPower2;
		var integralFrequentPart2 : Float = 0.5 * controlToStartMultiplyMainDiagonal * Math.sqrt(startToControlLenghtPower2) / diagonalLenghtPower2;
		var integralFrequentPart3 : Float = controlToStartMultiplyMainDiagonal / Math.sqrt(diagonalLenghtPower2) + 2 * Math.sqrt(startToControlLenghtPower2);
		...
		while(i < distances.length) {
			...
			var maxIterations : Float = 20;
			var time : Float = distance / curveLength;
			if(diagonalLenght < PRECISION)  {
...
while(((Math.abs(arcLength - distance) > PRECISION) && (maxIterations--)));
	...
	public function getTimeByDistance(distance : Float) : Float {
		...
		var curveLength : Float = this.length;
		if(__isSegment)  {
			if(distance <= 0)  {
				return 0;
			}
		...
		var distances : Array<Dynamic> = new Array<Dynamic>();
		distances.push(distance);
		var times : Array<Dynamic> = getTimesByDistances(distances);
		return times[0];
	}
	 ...
	 * @playerversion Flash 9.0
	 */
	public function setPoint(time : Float, newX : Float = undefined, newY : Float = undefined) : Void {
		if((Math.isNaN(newX) && Math.isNaN(newY)))  {
			return;
		}
		var invertedTime : Float = 1 - time;
		var timePower2 : Float = time * time;
		var invertedTimePower2 : Float = invertedTime * invertedTime;
		var timeMultiplyInvertedTime : Float = 2 * time * invertedTime;
	...
	public function angleOffset(value : Float, fulcrum : Point = null) : Void {
		...
		var controlLine : Line = new Line(fulcrum, controlPoint);
	...
	public function getExistedPointIterators(point : Point) : Array<Dynamic> {
		...
		diagonalVector.x = start.x - 2 * control.x + end.x;
		diagonalVector.y = start.y - 2 * control.y + end.y;
		var squareCoefficient : Float = diagonalVector.x;
		var linearCoefficient : Float = 2 * startToControlVector.x;
		var freeCoefficient : Float = start.x - point.x;
		var solutions : Array<Dynamic> = Equations.solveQuadraticEquation(squareCoefficient, linearCoefficient, freeCoefficient);
		if(!solutions)  {
	...
	public function getClosest(fromPoint : Point) : Float {
		...
		diagonalVector.x = start.x - 2 * control.x + end.x;
		diagonalVector.y = start.y - 2 * control.y + end.y;
		var fromPointToStartVector : Point = POINT2;
		fromPointToStartVector.x = startPoint.x - fromPoint.x;
		fromPointToStartVector.y = startPoint.y - fromPoint.y;
		var startToControlLenght : Float = startToControlVector.length;
		var startToControlLenghtPower2 : Float = startToControlLenght * startToControlLenght;
		var controlToStartMultiplyMainDiagonal : Float = startToControlVector.x * diagonalVector.x + startToControlVector.y * diagonalVector.y;
		var diagonalLenght : Float = diagonalVector.length;
		var diagonalLenghtPower2 : Float = diagonalLenght * diagonalLenght;
		var fromPointToStartMultiplyMainDiagonal : Float = fromPointToStartVector.x * diagonalVector.x + fromPointToStartVector.y * diagonalVector.y;
		var fromPointToStartMultiplyStartToControl : Float = startToControlVector.x * fromPointToStartVector.x + startToControlVector.y * fromPointToStartVector.y;
		var extremumTimes : Array<Dynamic>;
		var cubicCoefficient : Float;
		var squareCoefficient : Float;
		var linearCoefficient : Float;
		var freeCoefficient : Float;
		if(diagonalLenght > PRECISION)  {
			cubicCoefficient = 1;
	...
	public function getSegment(fromTime : Float = 0, toTime : Float = 1) : Bezier {
		...
		var baseMiddle : Point = Point.interpolate(segmentStart, segmentEnd, 0.5);
	...
	public function getTangentAngle(time : Float = 0) : Float {
		...
		diagonalVector.x = start.x - 2 * control.x + end.x;
		diagonalVector.y = start.y - 2 * control.y + end.y;
		var tangentX : Float = startToControlVector.x + diagonalVector.x * time;
		var tangentY : Float = startToControlVector.y + diagonalVector.y * time;
	...
	public function intersectionLine(target : Line) : Intersection {
		...
		diagonalVector.x = start.x - 2 * control.x + end.x;
		diagonalVector.y = start.y - 2 * control.y + end.y;
		var lineVector : Point = POINT2;
		lineVector.x = target.start.x - target.end.x;
		lineVector.y = target.start.y - target.end.y;
		var deltaStarts : Point = POINT3;
		deltaStarts.x = start.x - target.start.x;
		deltaStarts.y = start.y - target.start.y;
		var coefficientInPower2 : Float;
		var coefficientInPower1 : Float;
		var coefficientInPower0 : Float;
		var solutionsForCurve : Array<Dynamic>;
		var solutionForLine : Float;
		var solutionForCurve : Float;
		/*
		...
		if(Math.abs(lineVector.x) < PRECISION)  {
			...
			if(Math.abs(lineVector.y) < PRECISION)  {
				...
				i = 0;
				while(i < solutionsForCurve.length) {
			...
			if(Math.abs(lineVector.y) < PRECISION)  {
				...
				var normalizationCoefficient : Float = lineVector.y / lineVector.x;
	...
	public function intersectionBezier(target : Bezier) : Intersection {
		...
		var ax1 : Float = diagonalVector.x;
		var ay1 : Float = diagonalVector.y;
		var bx1 : Float = 2 * startToControlVector.x;
		var by1 : Float = 2 * startToControlVector.y;
		var cx1 : Float = startPoint.x;
		var cy1 : Float = startPoint.y;
		var ax2 : Float = targetDiagonalVector.x;
		var ay2 : Float = targetDiagonalVector.y;
		var bx2 : Float = 2 * targetStartToControlVector.x;
		var by2 : Float = 2 * targetStartToControlVector.y;
		var cx2 : Float = target.startPoint.x;
		var cy2 : Float = target.startPoint.y;
		var cx : Float = cx1 - cx2;
		var cy : Float = cy1 - cy2;
		// решение «в лоб»
		var part1 : Float = (ax2 * ay1 - ax1 * ay2);
		var part2 : Float = (-ay2 * bx1 + ax2 * by1);
		var part3 : Float = (bx1 * bx1 + 2 * ax1 * cx);
		var part4 : Float = (ay1 * bx2 - ax1 * by2);
		var part5 : Float = (by1 * by1 + 2 * ay1 * cy);
		var part6 : Float = (bx2 * bx2 + 2 * ax2 * cx);
		var part7 : Float = (bx1 * by1 + ax1 * cy);
		var part8 : Float = (bx2 * by1 - bx1 * by2);
		var part9 : Float = (by1 * cx + bx1 * cy);
		var part10 : Float = (-bx2 * by1 + bx1 * by2);
		var part11 : Float = (-by2 * cx + bx2 * cy);
		var part12 : Float = (-by2 * cx + bx2 * cy);
		var A : Float = -part1 * part1;
		var B : Float = -2 * part1 * part2;
		var C : Float = -ay2 * ay2 * part3 - ax2 * (by2 * part4 + ax2 * part5) + ay2 * (-ax1 * bx2 * by2 + ay1 * part6 + 2 * ax2 * part7);
		var D : Float = -2 * ay2 * ay2 * bx1 * cx + ay2 * (bx2 * part8 + 2 * ax2 * part9) + ax2 * (part10 * by2 - 2 * ax2 * by1 * cy);
		var E : Float = -ay2 * ay2 * cx * cx + ay2 * (bx2 * part11 + 2 * ax2 * cx * cy) - ax2 * (part12 * by2 + ax2 * cy * cy);
		var solutionsForCurve : Array<Dynamic> = Equations.solveEquation(A, B, C, D, E);
		...
		var bxn : Float = bx2 * cosa + by2 * sina;
		var cxn : Float = cx2 * cosa + cy2 * sina;
		...
		i = 0;
		while(i < solutionsForCurve.length) {
			var solutionForCurve : Float = solutionsForCurve[i];
			var solutionForTarget : Float;
			pointSolve = getPoint(solutionForCurve);
			var ox : Float = pointSolve.x * cosa + pointSolve.y * sina;
