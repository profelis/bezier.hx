...
class Intersection extends Dynamic {

	static public function isIntersectionPossible(current : Rectangle, target : Rectangle) : Bool {
	 ...
	 * 
	 **/
	public var currentTimes : Array<Dynamic>;
	/* *
	 * Массив, содержащий time-итераторы точек пересечения.
	 ...
	 * @playerversion Flash 9.0
	 **/
	public var targetTimes : Array<Dynamic>;
	/* *
	...
	public function translateCurrentIterators(originFigure : IParametric, finalFigure : IParametric) : Void {
		var correctedCurrentTimes : Array<Dynamic> = new Array<Dynamic>();
		var correctedTargetTimes : Array<Dynamic> = new Array<Dynamic>();
		var i : Int = 0;
		while(i < currentTimes.length) {
			var point : Point = originFigure.getPoint(currentTimes[i]);
			var iterators : Array<Dynamic> = finalFigure.getExistedPointIterators(point);
			var j : Int = 0;
		...
		i = 0;
		while(i < correctedCurrentTimes.length) {
	 ...
	 * @playerversion Flash 9.0
	 **/
	public function addIntersection(solutionForCurrent : Float, solutionForTarget : Float, currentIsSegment : Bool, targetIsSegment : Bool) : Intersection {
		var segmentRestrictionForCurve : Bool = (!currentIsSegment) || ((solutionForCurrent >= 0) && (solutionForCurrent <= 1));
		var segmentRestrictionForLine : Bool = (!targetIsSegment) || ((solutionForTarget >= 0) && (solutionForTarget <= 1));
